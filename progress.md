...
| 7. CLI/API workflows | In progress | CLI exposes baseline/confirmation counts, explicit terminal outcomes, stable process exit codes, and delegates search to the shared discovery service. `354ae74` guarantees one JSON terminal record for malformed onboarding input; `35200c9` closes SSE for all terminal phases; `5720ce0` persists repository-search diagnostics. |
| 8. Conformance and documentation | In progress | Workspace suite passes (244 tests, 3 intentional skips), all typechecks pass, Docker-gated Compose conformance passes, and public/API/runtime redaction coverage is present. Public-repository checks and remaining outcome/publication regression coverage remain |
