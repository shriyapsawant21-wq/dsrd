...
| 7. CLI/API workflows | In progress | CLI exposes baseline/confirmation counts, explicit terminal outcomes, stable process exit codes, and delegates search to the shared discovery service. `0c34465` and `aa75ec7` guarantee one JSON terminal record for direct-search and replay setup failures; `429239a` normalizes unexpected repository-search results to the closing `error` SSE phase; `5720ce0` persists repository-search diagnostics. |
| 8. Conformance and documentation | In progress | Workspace suite passes (255 tests, 3 intentional skips), all typechecks pass, Docker-gated Compose conformance passes, and public/API/runtime redaction coverage is present. Public-repository checks and remaining outcome/publication regression coverage remain. |
