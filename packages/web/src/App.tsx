...
export default function App() {
  ...
  return <div className="app" data-theme={theme}>
    ...
    {screen === "landing" && <main>
      ...
      <section className="upload-section"><div className="section-title">[ INITIALIZE_SEQUENCE ]</div><button className={`drop-zone ${dragging ? "dragging" : ""}`} onClick={() => input.current?.click()} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(e) => { e.preventDefault(); setDragging(false); selectProject(e.dataTransfer.files); }}><span className="corner">IN</span><Upload size={38} strokeWidth={2.4}/><strong>SELECT_COMPOSE_PROJECT_FOLDER</strong><span>OR_DRAG_AND_DROP_FOLDER_FILES</span><small>INCLUDES_COMPOSE_DOCKERFILES_AND_BUILD_CONTEXTS</small></button><input ref={input} className="sr-only" aria-label="Compose project folder" type="file" multiple {...({ webkitdirectory: "", directory: "" } as Record<string, string>)} onChange={(e) => selectProject(e.target.files)}/>{selectedFolder && <p className="selected-folder">PROJECT: {selectedFolder}</p>}{error && <p className="inline-error">{error}</p>}<p className="constraint">SELECT A FOLDER WITH ONE COMPOSE FILE // RELATIVE BUILD CONTEXTS ARE PRESERVED</p></section>
    </main>}
